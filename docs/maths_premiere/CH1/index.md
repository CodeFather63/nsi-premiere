...
| C est la réunion de A et B | C est l'événement (A ou B) | C = A ∪ B |
| C est l'intersection de A et B | C est l'événement (A et B) | C = A ∩ B |
| A et B sont disjoints | A et B sont des événements **incompatibles** | A ∩ B = ∅ |
| A et B sont complémentaires | A et B sont des événements **contraires** | B = A̅ |

### Loi de probabilité

**Définition 3**
Lorsqu'une expérience aléatoire comporte un nombre fini d'issues, on définit sur l'ensemble Ω = {ω₁, …, ωᵣ} des issues une **loi de probabilité** en se donnant une suite de nombres (p₁, …, pᵣ) vérifiant :
- pour tout i tel que 1 ≤ i ≤ r : pᵢ ≥ 0,
- p₁ + p₂ + … + pᵣ = 1.

**Définition 4**
Lorsque toutes les issues ont la même probabilité, on dit qu'il y a **équiprobabilité** ou que la loi est **équirépartie** sur Ω ou **uniforme**. Dans ce cas, si l'univers des issues Ω a r éléments, quelle que soit l'issue ωᵢ :
...
**Définition 5**
La **probabilité d'un événement** A est la somme de toutes les probabilités des issues appartenant à A. On pose P(∅) = 0.

**Propriété 1**
Lorsque toutes les issues sont équiprobables, on a :

P(A) = (nombre d'éléments de A) / (nombre d'éléments de Ω) = Card(A) / Card(Ω) = (nombre d'issues favorables à la réalisation de A) / (nombre d'issues possibles)
...
|---|---|---|
| A | A est un événement | 0 ≤ P(A) ≤ 1 |
| ∅ | événement impossible | P(∅) = 0 |
| Ω | événement certain | P(Ω) = 1 |
| A ∩ B = ∅ | A et B sont incompatibles | P(A ∪ B) = P(A) + P(B) |
| A̅ | A̅ est l'événement contraire de A | P(A̅) = 1 − P(A) |
| A ; B | A et B des événements quelconques | P(A ∪ B) = P(A) + P(B) − P(A ∩ B) |

---

## Fiche 1 — Probabilités Conditionnelles et arbres pondérés

Dans tout ce chapitre, Ω désigne un univers, A et B deux événements de Ω et P une probabilité sur Ω.
...
4. Comparer ce résultat avec P(D∩F) / P(F).
...
effectif de D∩F / effectif de F = fréquence de D∩F / fréquence de F
...
**Définition 1**
Si P(A) ≠ 0, **la probabilité de B sachant A**, notée P_A(B), est définie par :

P_A(B) = P(A∩B) / P(A)

**Remarque**
Si P(B) ≠ 0, on a de manière symétrique :

P_B(A) = P(B∩A) / P(B) = P(A∩B) / P(B)
...
1. Calculer P(E) et P(E∩M).
   ...
   - P(E) = card(E)/card(Ω) = 1095/1200 = 0,9125 ;
   - P(E∩M) = card(E∩M)/card(Ω) = 657/1200 = 0,5475.
2. En déduire P_E(M) avec la formule de la définition précédente.
   On en déduit que P_E(M) = P(E∩M)/P(E) = 0,5475/0,9125 = 0,6.
...
Arbre : depuis la racine, deux branches F (pondération 11/20) et F̅ ; depuis F, deux branches D (pondération 4/11) et D̅ ; depuis F̅, deux branches D et D̅ (pondérations à compléter).
...
2. a. Exprimer P_F(D) en fonction de P(F) et P(F∩D).
   b. En déduire P(F∩D) en fonction de P_F(D) et P(F).
...
**Remarques**
- Dans le cas de deux événements A et B de probabilités non nulles, on peut représenter la situation par un arbre A/A̅ puis B/B̅ (avec les pondérations P_A(B), P_A(B̅), P_A̅(B), P_A̅(B̅)), ou symétriquement par un arbre B/B̅ puis A/A̅ (avec les pondérations P_B(A), P_B(A̅), P_B̅(A), P_B̅(A̅)). C'est le contexte qui induira de représenter la situation par un arbre ou l'autre.
- Le premier point de la propriété illustre le fait que les événements A₁, A₂, … et Aₙ correspondant aux branches partant du premier nœud sont des événements disjoints, de probabilités non nulles et tels que A₁ ∪ A₂ ∪ … ∪ Aₙ = Ω.
  On dit alors que A₁, A₂, … et Aₙ forment une **partition de l'univers Ω**.

**Propriété 2**
Si P(A) ≠ 0 et P(B) ≠ 0, alors P(A∩B) = P(A) × P_A(B) = P(B) × P_B(A).
...
*Démonstration : Conséquence directe de la définition d'une probabilité conditionnelle.* ∎
...
1. Représenter la situation par un arbre.
   Pour le premier nœud, les deux possibilités sont R : « le légume choisi est rouge » et son événement contraire R̅ soit V : « le légume choisi est vert ». Il reste ensuite à distinguer tomates et poivrons pour les « deuxièmes nœuds ». Comme l'événement « le légume choisi est un poivron » n'est pas nommé par une lettre, on a utilisé A̅ pour le représenter dans l'arbre mais on aurait aussi pu introduire une nouvelle notation.

   Arbre : R (0,75) → A (0,7), A̅ (0,3) ; V (0,25) → A (0,2), A̅ (0,8).

2. Calculer P(R∩A).
   P(R∩A) = P(R) × P_R(A) = 0,75 × 0,7 = 0,525.

### Exercices

**Exercice 3.** A, B, C, D, E et H désignent des événements quelconques d'un univers Ω.
1. Trouver l'erreur dans l'arbre de probabilité suivant : arbre partant en A / B (racine), puis A → A̅, B̅ et B → A̅, B̅ (les branches secondaires reprennent les mêmes labels A̅/B̅ ce qui est incohérent).
2. Quelle(s) condition(s) doivent vérifier les événements C, D et E pour que l'arbre ci-dessous (racine → C (0,2) → H (0,1)/H̅ (0,9) ; racine → D (0,35) → H (0,15)/H̅ (0,85) ; racine → E (P(E)) → H (0,99)/H̅ (0,01)) soit un arbre pondéré correct ?

**Exercice 4.**
1. On considère deux événements R et S tels que P(R) = 1/4, P_R(S) = 5/6 et P_R̅(S̅) = 11/12.
...
**Exercice 5.** On considère deux événements A et B tels que P(A) = 0,1 et P(A∩B) = 0,06. Calculer P_A(B).

**Exercice 6.** On considère deux événements C et D tels que P(D) = 0,6 et P(C∩D̅) = 0,35. Calculer P_D̅(C).

**Exercice 7.** On considère deux événements incompatibles E et F de probabilités non nulles. Calculer P_E(F).

**Exercice 8.** On considère deux événements A et B tels que P(A) = 0,37, P(B) = 0,68 et P(A∪B) = 0,84. Calculer :
1. P_A(B)
2. P_B(A)
...
**Exercice 10.** On considère deux événements E et F tels que P(E) = 1/3 et P_E̅(F) = 7/12. Calculer :
1. P(E̅∩F)
2. P(E̅∩F̅)
...
1. Recopier et compléter l'arbre (racine → L / L̅ → chacun B/B̅).
2. Calculer P(L∩B) et interpréter cette probabilité dans les termes de l'énoncé.
...
2. Lire P(A), P(B̅), P(A∩B̅) et P(A̅∩B̅).
3. Calculer P_A(B), P_A(B̅), P_A̅(B) et P_A̅(B̅). On écrira les résultats sous forme de fractions irréductibles.
4. Recopier et compléter l'arbre en partant de A/A̅ puis B/B̅.
5. De même, recopier et compléter l'arbre en partant de B/B̅ puis A/A̅.
...
1. Calculer P(A), P(B), P(C), P(A∩B), P(A∩C) et P(B∩C).
...
**Exercice 19 - Trois à la suite.** A-t-on P(A∩B∩C) = P(A) × P_A(B) × P_B(C) ?
...
- Si P(A) ≠ 0 et P(A) ≠ 1 alors :

  P(B) = P(A∩B) + P(A̅∩B) = P(A) × P_A(B) + P(A̅) × P_A̅(B)

- De même, si A₁, A₂, … et Aₙ forment une partition de Ω, c'est-à-dire sont n évènements disjoints, de probabilités non nulles et tels que A₁ ∪ A₂ ∪ … ∪ Aₙ = Ω alors

  P(B) = P(A₁∩B) + P(A₂∩B) + … + P(Aₙ∩B) = P(A₁) × P_A₁(B) + P(A₂) × P_A₂(B) + … + P(Aₙ) × P_Aₙ(B)

*Démonstration :*
- On a B = (A∩B) ∪ (A̅∩B). De plus, les évènements A∩B et A̅∩B sont disjoints. Par conséquent, on a : P(B) = P(A∩B) + P(A̅∩B).
  On en déduit que : P(B) = P(A) × P_A(B) + P(A̅) × P_A̅(B).
- Le second point se démontre de même car les évènements Aₖ∩B pour k allant de 1 à n forment une partition de B. ∎
...
1. Dresser un arbre pondéré représentant la situation.
   ...
   Arbre : L (0,17) → B (0,906) / B̅ (0,094) ; E (0,31) → B (0,912) / B̅ (0,088) ; S (0,52) → B (0,918) / B̅ (0,082).

2. Quelle est la probabilité que la personne tirée au hasard ait obtenu le bac ?
   La formule des probabilités totales donne

   P(B) = P(L) × P_L(B) + P(E) × P_E(B) + P(S) × P_S(B) = 0,17 × 0,906 + 0,31 × 0,912 + 0,52 × 0,918 = 0,9141

3. Déterminer P_B̅(S).
   On sait que P_B̅(S) = P(B̅∩S) / P(B̅) où :
   - P(B̅∩S) = P(S) × P_S(B̅) = 0,52 × 0,082 = 0,04264 ;
   - P(B̅) = 1 − P(B) = 1 − 0,9141 = 0,0859.

   On en déduit donc que P_B̅(S) = 0,04264 / 0,0859 ≈ 0,496.

### Exercices

**Exercice 20.** On considère deux évènements A et B tels que P(A) = 0,8 et P(A∩B) = 0,48.
1. Montrer que P(A∩B̅) = 0,32.
2. Calculer P_A(B̅).

**Exercice 21.** On considère deux évènements E et F tels que P(E) = 0,4 et P(E̅∩F̅) = 0,12. Calculer P_E̅(F).

**Exercice 22.** On considère deux évènements A et B tels que P(A) = 0,45 ; P(B) = 0,6 et P(A∪B) = 0,71. Calculer :
1. P_A(B)
2. P_A(B̅)
3. P_B̅(A)
4. P_B̅(A̅)
...
2. Calculer P(F̅∩V), P(V) et P_V(F̅).

**Exercice 27.** Compléter l'arbre 2 en utilisant l'arbre 1 :
Arbre 1 : A (1/5) → B (1/8) / B̅ (7/8) ; A̅ (4/5) → B (4/9) / B̅ (5/9).
Arbre 2 : (à partir de B/B̅ puis A/A̅, à compléter).
...
1. Dans l'urne de l'exercice 18, on tire successivement et sans remise 2 boules.
   On note les événements R₁ : « la première boule tirée est rouge », R₂ : « la deuxième boule tirée est rouge », et de même J₁, J₂, V₁ et V₂.
   a. À l'aide d'une partition convenable, calculer P(R₂).
   b. Calculer de même P(J₂) et P(V₂).
   c. Comparer les nombres trouvés à P(R₁) ; P(J₁) et P(V₁). Remarque ?
2. Dans la même urne, on réalise à présent trois tirages successifs sans remise. Calculer la probabilité de R₃ : « la troisième boule tirée est rouge ». Remarque ?
...
A affirme : « P(G) = 1/3 » ; B répond : « mais non, P(G) < 1/4 » ; enfin, C prétend : « P(G) > 1/2 ».
...
**Remarque**
Attention à ne pas confondre « indépendants » et « incompatibles » qui est synonyme de disjoints c'est-à-dire que A∩B = ∅ et non pas P(A∩B) = P(A) × P(B).
...
On détermine puis on compare P(A) × P(L) et P(A∩L).
D'après l'énoncé, P(L) = 0,71. De plus, P(A) = 1 − 0,63 = 0,37, donc : P(A) × P(L) = 0,37 × 0,71 = 0,2627.

D'autre part, d'après l'énoncé, P_L(A) = 0,37, donc :

P(A∩L) = P(L) × P_L(A) = 0,71 × 0,37 = 0,2627

Comme P(A) × P(L) = P(A∩L), on en déduit que A et L sont indépendants.

**Propriété 4**
Si P(A) ≠ 0 (ou P(B) ≠ 0) alors :
...
si et seulement si P(A) × P_A(B) = P(A) × P(B)
si et seulement si P_A(B) = P(B). ∎
...
**Propriété 5**
Si A et B sont deux évènements indépendants alors A̅ et B sont indépendants.

*Démonstration :*
Soit A et B deux évènements indépendants.
Montrons que P(A̅∩B) = P(A̅) × P(B).
D'après la formule des probabilités totales, on a : P(B) = P(A̅∩B) + P(A∩B)
D'où : P(A̅∩B) = P(B) − P(A∩B).
De plus, comme A et B sont indépendants, on a P(A∩B) = P(A) × P(B).
On obtient ainsi :

P(A̅∩B) = P(B) − P(A) × P(B) = (1 − P(A)) × P(B) = P(A̅) × P(B)

D'où le résultat. ∎

**Exemple**
Dans l'exemple précédent, les événements A : « la personne a 55 ans ou plus » et L̅ : « la personne ne porte pas de lunettes » sont donc également indépendants.

**Remarque**
Plus généralement, si A et B sont indépendants alors :
- A̅ et B sont indépendants,
- A̅ et B̅ sont indépendants,
- A et B̅ sont indépendants.

### Exercices

**Exercice 33.** On considère deux évènements indépendants A et B tels que P(A) = 0,15 et P(A∩B) = 0,085. Calculer P(B).

**Exercice 34.** On considère deux évènements indépendants E et F tels que P(F̅) = 0,53 et P(E∩F) = 0,25. Calculer P(E).

**Exercice 35.** On considère deux évènements indépendants C et D tels que P(C∪D) = 0,23 et P(C) = 0,11. Calculer P(D).
...
1. Soit A et B deux événements indépendants. Montrer qu'il en est de même pour :
   ...
   b. B et A̅
   c. A̅ et B̅
2. Deux archers tirent simultanément. Les deux événements « A atteint la cible » et « B atteint la cible » sont indépendants et de probabilités 4/5 et 7/8.
...
Le moteur central a pour sécurité P₁ = 199/200, les moteurs d'ailes ont chacun la sécurité P₂ = 9/10 et les moteurs fonctionnent ou non indépendamment les uns des autres. Quel est la probabilité que l'avion tombe ?
...
2. Comparer P(A∩B∩C) et P(A)P(B)P(C).
...
1. Recopier et compléter sur la copie l'arbre de probabilité suivant en utilisant les données de l'exercice (racine → M / M̅ → chacun T/T̅).
2. Justifier que P(M̅∩T) = 0,0198.
...
- B₁ l'événement « le cookie provient de la boulangerie 1 »,
- B₂ l'événement « le cookie provient de la boulangerie 2 »,
- B₃ l'événement « le cookie provient de la boulangerie 3 »,
...
- P_B₂(C) = 0,4 où P_B₂(C) est la probabilité conditionnelle de C sachant B₂ ;
- la probabilité que le cookie soit aux noisettes sachant qu'il provient de la troisième boulangerie est de 0,3.

L'arbre pondéré ci-dessous correspond à la situation et donne une information supplémentaire : le nombre 0,6 sur la branche de B₁ à C.

Arbre : B₁ (à compléter) → C (0,6) / C̅ ; B₂ (à compléter) → C / C̅ ; B₃ (à compléter) → C / C̅.

1. Exprimer par une phrase l'information donnée par le nombre 0,6 sur la branche de B₁ à C.
2. Recopier et compléter sur la copie l'arbre pondéré ci-dessus.
3. Définir par une phrase l'événement B₁∩C et calculer sa probabilité.
...
- C̅ : l'événement l'adolescent n'a jamais consommé du cannabis.

Sur les lycéens qui ont participé à cette enquête on constate que la probabilité qu'un adolescent ait répondu « oui » est de 3/5, soit p(O) = 3/5. On veut déterminer la probabilité, notée p, qu'un adolescent ait déjà consommé du cannabis. On a donc p(C) = p.

1. Justifier que la probabilité qu'un adolescent ait répondu « oui » sachant qu'il n'a jamais consommé de cannabis est 1/6.
2. On a ci-dessous l'arbre de probabilités représentant la situation (racine → C (probabilité p) → O / N (pondération 1/6 indiquée) ; racine → C̅ → O (pondération 1/6) / N). Compléter cet arbre.
...
1. a. Calculer la probabilité de l'événement : « toutes les boules tirées sont de même couleur ».
   ...
   c. En déduire que les probabilités P(A∩B), P(A), P(B) sont : P(A∩B) = n/2^n, P(A) = 1 − 1/2^(n-1) et P(B) = (n+1)/2^n.
2. Montrer que P(A∩B) = P(A) × P(B) si, et seulement si :

   2^(n-1) = n + 1

3. Soit u la fonction définie sur ℕ \ {0, 1} par,
...
On considère une population (génération 0) dans laquelle les proportions respectives de ces trois génotypes sont p₀, q₀ et r₀. On admet que les couples se forment au hasard quant aux génotypes considérés (appariement aléatoire).

2. Exprimer, en fonction de p₀, q₀ et r₀ la probabilité p₁ qu'un enfant de la génération 1 ait le génotype AA, puis celles notées q₁ et r₁, qu'il ait le génotype Aa ou aa.
3. Montrer que p₁, q₁ et r₁ s'expriment seulement à l'aide de α = p₀ − r₀. En déduire p₂, q₂ et r₂ (ces mêmes proportions à la génération 2) et conclure.

**Exercice 51. Loi de Hardy Weinberg pour les allèles**
Les notations de l'exercice sont celles de l'exercice précédent. On note (uₙ) et (vₙ) les proportions des allèles A et a dans la population.

1. Montrer que uₙ = pₙ + qₙ/2 et vₙ = rₙ + qₙ/2.
2. En déduire que les nombres uₙ et vₙ sont indépendants de n et les exprimer à l'aide de α.
